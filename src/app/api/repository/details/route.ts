...
import { ManifertAccepted } from "../manifest/manifest.entity";
/**
    ...
    if (response.status === 200) {
        ...
        if (!registry) {
            ...
            try {
                ...
                const configBlob = await app.RepositoryService.getConfigBlog(route, image, token);
                const details = await app.RepositoryService.checkBlobRepository(route, image, token, configBlob);
                response = Response.json({
                    manifest: configBlob?.manifest,
                    details,
                });
            } catch (err: unknown) {
